...
export const textMessageController = async (req, res) => {
  try {
    ...
    if (req.user.credits < 1) return res.json({ success: false, message: "Not enough credits" });
...
export const imageMessageController = async (req, res) => {
  try {
    const userId = req.user._id;
    if (req.user.credits < 2) return res.json({ success: false, message: "Not enough credits" });
